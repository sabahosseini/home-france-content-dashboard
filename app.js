...
function renderProposals(){
 ...
 document.querySelectorAll(".delete-proposal").forEach(b=>b.onclick=()=>deleteProposal(b.dataset.id));
}
...
async function deleteProposal(id){
 const item=proposals.find(x=>x.id===id);if(!item)return;
 if(!window.confirm(`Delete ${item.code} · ${item.subject}? This cannot be undone.`))return;
 const {error}=await db.from("proposals").delete().eq("id",id);
 if(error)return toast(error.message);
 await load();toast(`${item.code} deleted`);
}
